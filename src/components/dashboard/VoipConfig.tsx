...
const voipConfigSchema = z.object({
    ...
    sip_host: z.string().min(1, "SIP hostname is required"),
    sip_port: z.string().refine((val) => {
...
export function VoipConfig() {
    ...
    React.useEffect(() => {
        if (tenantData) {
            reset({
                ...
                sip_host: tenantData.sip?.host || "",
                sip_port: String(tenantData.sip?.port || ""),
    ...
    const onSubmit = (data: VoipFormData) => {
        ...
        const port = Number.parseInt(data.sip_port)
        ...
        if (tenantData) {
            ...
            if (data.sip_host !== tenantData.sip?.host) {
                sipChanges.host = data.sip_host
            }
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="bg-white shadow rounded-lg p-6">
            ...
            <div className="space-y-4">
                <div>
                    ...
                    <select
                        ...
                        className={`mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 ${
                            validationErrors.voip_system_type ? "border-red-500" : ""
                        }`}
                        disabled={!isEditing}
                        {...register("voip_system_type")}
                    >
                        <option value="">Select VoIP System Type</option>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        <label htmlFor="sip_host" className="block text-sm font-medium text-gray-700">
                            SIP Hostname
                        </label>
                        <Input id="sip_host" {...register("sip_host")} disabled={!isEditing}
                               error={!!validationErrors.sip_host}/>
                        {validationErrors.sip_host &&
                            <p className="mt-1 text-sm text-red-600">{validationErrors.sip_host}</p>}
                    </div>
                    <div>
                        <label htmlFor="sip_port" className="block text-sm font-medium text-gray-700">
                            SIP Port
                        </label>
                        <Input
                            id="sip_port"
                            ...
                            {...register("sip_port")}
                            disabled={!isEditing}
                            error={!!validationErrors.sip_port}
                        />
                        {validationErrors.sip_port &&
                            <p className="mt-1 text-sm text-red-600">{validationErrors.sip_port}</p>}
                    </div>
                    <div>
                        ...
                        <select
                            ...
                            className={`mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 ${
                                validationErrors.transport_protocol ? "border-red-500" : ""
                            }`}
                            disabled={!isEditing}
